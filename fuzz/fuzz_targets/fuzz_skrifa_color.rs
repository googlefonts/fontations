...
fn do_color_glyph_things(
    ...
    let location = create_location(font, axis_positions);
    let _ = color_glyph.bounding_box(&location, size);
    let _ = color_glyph.paint(&location, &mut NopPainter);
    Ok(())
}

fn do_skrifa_things(data: &[u8], size: Size, axis_positions: &[f32]) -> Result<(), Box<dyn Error>> {
    ...
    for glyph_id in charmap.mappings().map(|(_cp, gid)| gid) {
        ...
        {
            let _ = do_color_glyph_things(&font, &colrv1_glyph, size, axis_positions);
        } else if let Some(colrv0_glyph) =
            color_glyphs.get_with_format(glyph_id, ColorGlyphFormat::ColrV0)
        {
            let _ = do_color_glyph_things(&font, &colrv0_glyph, size, axis_positions);
        }
    }
...
fuzz_target!(|data: &[u8]| {
    ...
    let scenarios = fuzz_sizes()
        .into_iter()
        .flat_map(|size| {
            [vec![0.0], random_position.clone()]
                .into_iter()
                .map(move |pos| (size, pos))
        })
        .collect::<Vec<_>>();

    for (size, pos) in scenarios {
        let _ = do_skrifa_things(data, size, &pos);
    }
});
