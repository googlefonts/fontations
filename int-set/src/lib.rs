...
//!
//! For a type to be stored in the IntSet it must implement the IntSetMember trait, and all
//! unique values of that type must be able to be mapped to and from a unique u32 value.
//! See the IntSetMember trait for more information.
...
///
/// 3. all_values_iter() must iterate over all values in T in sorted order (ascending).
///
/// from_mapped() will only ever be called with u32 values that were generated by a
/// call to to_mapped() with a valid value of T. So it doesn't need to correctly handle values
/// that are outside the domain of T.
pub trait IntSetMember<T> {
    fn to_mapped(&self) -> u32;
    fn from_mapped(member: InDomain) -> T;
    fn all_values_sorted_iter() -> impl Iterator<Item = u32>;
}

/// Marks a mapped value as being in the domain of T for IntSetMember<T>. See IntSetMember
...
impl<T: IntSetMember<T>> Default for IntSet<T> {
    fn default() -> IntSet<T> {
        IntSet::empty()
    }
}

impl<T: IntSetMember<T>> IntSet<T> {
    ...
    pub fn insert_range(&mut self, range: RangeInclusive<T>) {
        let range = range.start().to_mapped()..=range.end().to_mapped();
        match &mut self.0 {
    ...
    pub fn extend_unsorted<U: IntoIterator<Item = T>>(&mut self, iter: U) {
        let iter = iter.into_iter().map(|v| v.to_mapped());
        match &mut self.0 {
...
impl<T: IntSetMember<T>> FromIterator<T> for IntSet<T> {
...
impl<T: IntSetMember<T>> Extend<T> for IntSet<T> {
    ...
    T: IntSetMember<T>,
{
    ...
    let mut all_values = T::all_values_sorted_iter();

    std::iter::from_fn(move || {
        if !exclusive {
            let ret = cur_value;
            cur_value = values.next();
            return ret.map(|v| T::from_mapped(InDomain { value: v }));
        }

        for index in all_values.by_ref() {
            let index = index.to_mapped();
            loop {
                let Some(skip) = cur_value else {
                    // No-longer any values to skip, can freely return index
                    return Some(T::from_mapped(InDomain { value: index }));
                };
...
impl IntSetMember<u32> for u32 {
    fn to_mapped(&self) -> u32 {
        *self
    }

    fn from_mapped(member: InDomain) -> u32 {
        member.value()
    }
...
impl IntSetMember<u16> for u16 {
    fn to_mapped(&self) -> u32 {
        *self as u32
    }

    fn from_mapped(member: InDomain) -> u16 {
        member.value() as u16
    }
...
mod test {
    ...
    impl IntSetMember<EvenInts> for EvenInts {
        fn to_mapped(&self) -> u32 {
            self.0 as u32
        }

        fn from_mapped(member: InDomain) -> EvenInts {
            EvenInts(member.value as u16)
        }
