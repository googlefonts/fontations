...
use parse::{Item, Items, Record, Table, TableFormat};
...
pub fn generate_parse_module(code: &str) -> Result<proc_macro2::TokenStream, syn::Error> {
    ...
    for item in &items.items {
        let item_code = match item {
            Item::Table(item) => generate_table_impl(item)?,
            Item::Record(item) => generate_record_impl(item)?,
...
fn generate_table_impl(item: &Table) -> syn::Result<proc_macro2::TokenStream> {
    Ok(Default::default())
    //todo!()
}

fn generate_record_impl(item: &Record) -> syn::Result<proc_macro2::TokenStream> {
    let name = &item.name;
