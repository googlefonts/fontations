...
impl<'a> Outlines<'a> {
    ...
    fn component_coords(
        ...
        self.axis_indices(
            axis_indices_index as usize,
            num_axes,
            &mut scratch.axis_indices,
        )?;
...
fn compute_sparse_region_scalar(region: &SparseVariationRegion<'_>, coords: &[F2Dot14]) -> f32 {
    ...
    for axis in region.region_axis_offsets() {
        ...
        if coord < start || coord > end {
            ...
            let numerat = (coord.to_bits() - start.to_bits()) as f32;
            let denom = (peak.to_bits() - start.to_bits()) as f32;
            scalar = scalar * numerat / denom;
        } else {
            // Use raw bits - scale factors cancel in the ratio
            let numerat = (end.to_bits() - coord.to_bits()) as f32;
