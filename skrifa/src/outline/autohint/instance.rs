...
impl GlyphStyles {
    ...
    pub fn new(outlines: &OutlineGlyphCollection) -> Self {
        if let Some(scaler) = outlines.base_outlines() {
            let glyph_count = scaler
                .font
                .maxp()
...
impl Instance {
    pub fn new(
        ...
        let is_fixed_width = font
            ...
            .unwrap_or_default();
        let is_italic = font.attributes().style != Style::Normal;
        Self {
            styles,
            metrics,
            target,
    ...
    pub fn draw(
        ...
        let base_outlines = glyph.base_outlines();
        ...
        let metrics = self
            .metrics
            .get(&base_outlines.font, coords, &self.styles.0, glyph_id)
            .ok_or(DrawError::GlyphNotFound(glyph_id))?;
        ...
        let (x_scale, edge_metrics) = super::latin::hint_outline(&mut outline, &metrics, &scale);
        let h_advance = base_outlines.advance_width(glyph_id, coords);
        let mut pp1x = 0;
        let mut pp2x = fixed_mul(h_advance, x_scale);
        ...
        if !is_light {
            if let (true, Some(edge_metrics)) = (scale.flags & Scale::NO_ADVANCE == 0, edge_metrics)
            {
