...
impl GlyphStyle {
    ...
    pub(super) fn set_gsub_output(&mut self) {
        self.0 |= Self::GSUB_OUTPUT
    }

    pub(super) fn clear_shaping(&mut self) {
        self.0 &= !Self::GSUB_OUTPUT;
    }
    ...
    pub(super) fn maybe_assign_gsub_output_style(&mut self, style: &StyleClass) -> bool {
        ...
        if self.0 & Self::GSUB_OUTPUT != 0 && self.is_unassigned() {
            self.clear_shaping();
