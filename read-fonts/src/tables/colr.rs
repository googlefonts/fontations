...
impl<'a> Colr<'a> {
    ...
    pub fn v0_closure_glyphs(
        ...
        if let Some(Ok(records)) = self.base_glyph_records() {
...
impl<'a> Colrv1ClosureContext<'a> {
    ...
    pub fn dispatch(&mut self, paint: &Paint) {
    ...
    pub fn add_layer_indices(&mut self, first_layer_index: u32, last_layer_index: u32) {
        self.layer_indices
            .insert_range(first_layer_index..=last_layer_index);
    }

    pub fn add_palette_index(&mut self, palette_index: u16) {
        self.palette_indices.insert(palette_index);
    }

    pub fn add_variation_indices(&mut self, var_index_base: u32, num_vars: u8) {
    ...
    pub fn add_glyph_id(&mut self, gid: GlyphId16) {
...
impl<'a> PaintColrLayers<'a> {
    fn v1_closure(&self, c: &mut Colrv1ClosureContext) {
        ...
        if let Some(Ok(layer_list)) = c.colr.layer_list() {
...
impl<'a> PaintColrGlyph<'a> {
    fn v1_closure(&self, c: &mut Colrv1ClosureContext) {
        ...
        if let Some(Ok(list)) = c.colr.base_glyph_list() {
            let records = list.base_glyph_paint_records();
            let record = match records.binary_search_by(|rec| rec.glyph_id().cmp(&glyph_id)) {
                Ok(ix) => &records[ix],
                _ => return,
...
impl Clip {
    fn v1_closure(&self, c: &mut Colrv1ClosureContext, clip_list: &ClipList) {
        if let Ok(clip_box) = self.clip_box(clip_list.offset_data()) {
