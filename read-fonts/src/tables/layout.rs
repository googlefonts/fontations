...
impl<'a> Sanitize<'a> for FeatureParams<'a> {
    ...
    fn read_fast(data: FontData<'a>, args: Self::Args) -> Option<Self> {
        match args {
            t if t == Tag::new(b"size") => SizeParams::read_fast(data, ()).map(Self::Size),
            t if &t.to_raw()[..2] == b"ss" => {
                StylisticSetParams::read_fast(data, ()).map(Self::StylisticSet)
            }
            t if &t.to_raw()[..2] == b"cv" => {
                CharacterVariantParams::read_fast(data, ()).map(Self::CharacterVariant)
            }
            // NOTE: what even is our error condition here? an offset exists but
            // we don't know the tag?
            _ => None,
