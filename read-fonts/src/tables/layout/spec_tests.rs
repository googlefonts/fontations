...
fn example_1_scripts() {
    ...
    assert_eq!(
        table.script_records().get(0).unwrap().script_tag(),
        Tag::new(b"hani")
    );
    assert_eq!(
        table.script_records().get(1).unwrap().script_tag(),
        Tag::new(b"kana")
    );
    assert_eq!(
        table.script_records().get(2).unwrap().script_tag(),
        Tag::new(b"latn")
    );
}
...
fn example_2_scripts_and_langs() {
    ...
    let urdu_record = table.lang_sys_records().get(0).unwrap();
    assert_eq!(urdu_record.lang_sys_tag(), Tag::new(b"URD "));
    let urdu_sys = urdu_record.lang_sys().unwrap();
...
fn example_3_featurelist_and_feature() {
    ...
    let turkish_liga_record = table.feature_records().get(0).unwrap();
    let feature = turkish_liga_record.feature().unwrap();
