...
impl<'a> ScriptList<'a> {
    ...
    /// [`script_tags_from_unicode`] function to generate the appropriate set
...
pub fn script_tags_from_unicode(unicode_script: Tag) -> ScriptTags {
...
fn new_tag_from_unicode(unicode_script: Tag) -> Option<Tag> {
    let bytes = unicode_script.to_be_bytes();
    let tag_bytes = match &bytes {
        // Bengali
        b"Beng" => b"bng2",
        // Devanagari
...
mod tests {
    ...
    fn simple_script_tag_from_unicode() {
        ...
        for unicode_tag in unicode_tags {
            ...
            let result = script_tags_from_unicode(Tag::new(unicode_tag));
    ...
    fn select_scripts_from_unicode() {
        ...
        let hebr = gsub_scripts
            .select(&script_tags_from_unicode(Tag::new(b"Hebr")))
            .unwrap();
        assert_eq!(
            hebr,
            SelectedScript {
        ...
        let beng = gsub_scripts
            .select(&script_tags_from_unicode(Tag::new(b"Beng")))
            .unwrap();
        assert_eq!(
            beng,
            SelectedScript {
