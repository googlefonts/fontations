...
use super::{
    ...
    ClassSequenceRule, ClassSequenceRuleSet, CoverageTable, FeatureList, GlyphId, LangSys,
    ReadError, Script, ScriptList, SequenceContext, SequenceContextFormat1, SequenceContextFormat2,
    SequenceContextFormat3, SequenceLookupRecord, SequenceRule, SequenceRuleSet, Tag,
};
...
impl<'a> LookupClosureCtx<'a> {
    ...
    // return true if lookup limit exceeded or lookup visited
    pub(crate) fn lookup_visited(&mut self, lookup_index: u16) -> bool {
...
impl LookupClosure for ContextFormat2<'_> {
    fn intersects(&self, glyph_set: &IntSet<GlyphId>) -> Result<bool, ReadError> {
        ...
        let retained_glyph_classes = input_class_def.intersect_classes(glyph_set);
        ...
        for rule_set in self.rule_sets().enumerate().filter_map(|(c, rule_set)| {
            ...
            for rule in rule_set?.rules() {
                if rule?.matches_classes(&retained_glyph_classes) {
                    return Ok(true);
                }
            }
        }
