        ...
        loop {
            ...
            if lookups.is_inverted() {
                for i in 0..num_lookups {
                    ...
                    let offset = lookup_offsets.get_offset(i as usize)?;
                    if offset.is_null() {
                        continue;
                    }
...
impl GlyphClosure for SubstitutionLookup<'_> {
    fn closure_glyphs(
        ...
        subtables.closure_glyphs(ctx, lookup_list, lookup_index)
    }
    ...
    for Subtables<'a, T, Ext>
{
    fn closure_glyphs(
        ...
        self.iter_as_nullable().try_for_each(|t| {
            if let Some(t) = t.transpose()? {
...
impl GlyphClosure for LigatureSubstFormat1<'_> {
    fn closure_glyphs(
        ...
        for idx in lig_set_idxes {
            let offset = ligs.get_offset(idx)?;
            if offset.is_null() {
                continue;
            }
            ...
            })
        {
            ...
            for rule in rule_set?.rules() {
                ...
                for lookup_record in rule.lookup_records() {
                    ...
                    let lookup_offset = lookups.get_offset(lookup_index as usize)?;
                    if lookup_offset.is_null() {
                        continue;
                    }
                    ...
                    let lookup = lookups.get(lookup_index as usize)?;
...
impl SubstitutionLookupList<'_> {
    pub fn closure_lookups(
        ...
        for idx in lookup_indices.iter() {
            if lookups.get_offset(idx as usize)?.is_null() {
                continue;
            }
