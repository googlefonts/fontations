...
//! from HarfBuzz.
//!
//! The basic idea is straightforward: instead of doing bounds checks whenever
//! a field is accessed (or an offset is chased) we validate an entire font
//! table (recursively, including its subtables) once, up front, and then elide
//! subsequent bounds checking.
//!
//! # The post-sanitize invariant
//!
//! Tables in modules annotated with `#![sanitize]` (in the codegen inputs)
//! rely on the following invariant: *a table is only ever constructed from
//! data that has been validated by that table's [`Sanitize`] implementation.*
//!
//! Concretely, such a table is only constructed via one of:
//!
//! - [`FontRead::read`], which runs [`Sanitize::sanitize`] (walking the full
//!   graph of subtables with a checked cursor, verifying that every field and
//!   every offset target is in bounds) before calling [`FastRead::fast_read`];
//! - [`fast_resolve`][FastResolveOffset::fast_resolve], called with the data
//!   of a parent table that was itself sanitized (sanitize already descended
//!   into the same offset);
//! - `Default`, which is backed by a static pool of zeroed bytes whose size
//!   is verified against the table's `MIN_SIZE` by a generated const assert
//!   (all-zero data means all counts are zero and all version-gated fields
//!   are absent, so all field ranges fall within `MIN_SIZE`).
//!
//! Under this invariant, every byte range produced by a generated
//! `*_byte_range()` method and every non-null offset is in bounds of the
//! table's data, which is what justifies the `unsafe` unchecked reads in the
//! generated getters and in offset resolution. Note that this also means the
//! generated byte-range arithmetic must always agree with what the
//! corresponding `Sanitize` impl validated: any divergence between the two in
//! codegen becomes undefined behaviour rather than a wrong-but-safe answer
//! (debug assertions in the unchecked methods act as a tripwire for this).
//!
//! [hb_sanitize]: https://github.com/harfbuzz/harfbuzz/blob/main/src/hb-sanitize.hh
//! [`FontRead::read`]: crate::FontRead::read
...
///
/// Calling `fast_read` with data that has not been validated by the
/// corresponding [`Sanitize`] implementation violates the invariant that the
/// `unsafe` unchecked reads in the generated getters rely on (see the
/// [module docs](self)) and can result in undefined behaviour.
...
// `()` is used as a placeholder type argument for generic tables (like
// `Lookup<()>`) when the concrete subtable type is unknown or unimportant.
// Sanitizing it is a no-op: the offsets themselves are still checked to be
// in bounds of the parent, but the subtable contents are not validated.
impl ReadArgs for () {
    type Args = ();
}

impl Sanitize for () {
    fn sanitize(_ctx: &mut SanitizeContext<'_>, _args: ()) -> Result<(), ReadError> {
        Ok(())
    }
}

impl FastRead<'_> for () {
    fn fast_read(_data: FontData<'_>, _args: ()) -> Self {}
}
...
impl<O: Offset> FastResolveOffset for O {
    fn fast_resolve<'a, T: FastRead<'a> + Default>(
        ...
        match self.non_null() {
            // SAFETY: fast_resolve is only called with the data of a table
            // that has already been sanitized; sanitize verified that every
            // non-null offset reachable from that table is in bounds of its
            // data. See the module docs.
            Some(off) => Ok(T::fast_read(unsafe { data.split_off_unchecked(off) }, args)),
            None => Ok(T::default()),
        }
    }
}
...
impl<O: Offset> FastResolveNullableOffset for Nullable<O> {
    fn fast_resolve<'a, T: FastRead<'a> + Default>(
        ...
        self.offset()
            ...
            // non-null offset reachable from that table is in bounds of its
            // data. See the module docs.
            .map(|off| Ok(T::fast_read(unsafe { data.split_off_unchecked(off) }, args)))
    }
}
