...
use crate::sanitize::{FastRead, FastResolveNullableOffset, FastResolveOffset};
...
/// An array of offsets that resolves using [`FastRead`] (post-sanitize).
///
...
/// An array of nullable offsets that resolves using [`FastRead`] (post-sanitize).
///
...
where
    O: Scalar + Offset + Into<crate::traversal::OffsetType>,
    T: ReadArgs + FastRead<'a> + Default + crate::traversal::SomeTable<'a> + 'a,
    T::Args: Copy + 'static,
{
    fn type_name(&self) -> &str {
        let full_name = std::any::type_name::<T>();
