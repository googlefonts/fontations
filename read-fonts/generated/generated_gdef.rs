    ...
    fn read_fast(data: FontData<'a>, _args: ()) -> Option<Self> {
        ...
        match format {
            CaretValueFormat1::FORMAT => {
                CaretValueFormat1::read_fast(data, ()).map(CaretValue::Format1)
            }
            CaretValueFormat2::FORMAT => {
                CaretValueFormat2::read_fast(data, ()).map(CaretValue::Format2)
            }
            CaretValueFormat3::FORMAT => {
                CaretValueFormat3::read_fast(data, ()).map(CaretValue::Format3)
            }
            _ => None,
        }
    }
