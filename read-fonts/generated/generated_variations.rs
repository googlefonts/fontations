...
impl<'a> Sanitize<'a> for TupleVariationHeader<'a> {
    ...
    fn read_fast(data: FontData<'a>, args: u16) -> Option<Self> {
        let axis_count = args;
        read_fast_impl!(Self { data, axis_count })
    }
}
...
impl<'a> Sanitize<'a> for DeltaSetIndexMap<'a> {
    ...
    fn read_fast(data: FontData<'a>, _args: ()) -> Option<Self> {
        let format = data.read_at::<u8>(0usize).ok()?;
        match format {
            DeltaSetIndexMapFormat0::FORMAT => {
                DeltaSetIndexMapFormat0::read_fast(data, ()).map(DeltaSetIndexMap::Format0)
            }
            DeltaSetIndexMapFormat1::FORMAT => {
                DeltaSetIndexMapFormat1::read_fast(data, ()).map(DeltaSetIndexMap::Format1)
            }
            _ => None,
        }
    }
