...
impl<'a> PaintVarLinearGradient<'a> {
    ...
    /// Start point (p╬ô├⌐├ç) x coordinate. For variation, use
    /// varIndexBase + 0.
    pub fn x0(&self) -> FWord {
        let range = self.shape.x0_byte_range();
        self.data.read_at(range.start).unwrap()
    }
    /// Start point (p╬ô├⌐├ç) y coordinate. For variation, use
    /// varIndexBase + 1.
    pub fn y0(&self) -> FWord {
        let range = self.shape.y0_byte_range();
        self.data.read_at(range.start).unwrap()
    }
    /// End point (p╬ô├⌐├╝) x coordinate. For variation, use varIndexBase
    /// + 2.
    pub fn x1(&self) -> FWord {
        let range = self.shape.x1_byte_range();
        self.data.read_at(range.start).unwrap()
    }
    /// End point (p╬ô├⌐├╝) y coordinate. For variation, use varIndexBase
    /// + 3.
    pub fn y1(&self) -> FWord {
        let range = self.shape.y1_byte_range();
        self.data.read_at(range.start).unwrap()
    }
    /// Rotation point (p╬ô├⌐├⌐) x coordinate. For variation, use
    /// varIndexBase + 4.
    pub fn x2(&self) -> FWord {
        let range = self.shape.x2_byte_range();
        self.data.read_at(range.start).unwrap()
    }
    /// Rotation point (p╬ô├⌐├⌐) y coordinate. For variation, use
    /// varIndexBase + 5.
    pub fn y2(&self) -> FWord {
        let range = self.shape.y2_byte_range();
        self.data.read_at(range.start).unwrap()
...
impl<'a> PaintRotate<'a> {
    ...
    /// Rotation angle, 180Γö¼Γûæ in counter-clockwise degrees per 1.0 of
    /// value.
...
impl<'a> PaintSkew<'a> {
    ...
    /// Angle of skew in the direction of the x-axis, 180Γö¼Γûæ in
    /// counter-clockwise degrees per 1.0 of value.
    pub fn x_skew_angle(&self) -> F2Dot14 {
        let range = self.shape.x_skew_angle_byte_range();
        self.data.read_at(range.start).unwrap()
    }
    /// Angle of skew in the direction of the y-axis, 180Γö¼Γûæ in
    /// counter-clockwise degrees per 1.0 of value.
