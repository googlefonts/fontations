...
use write_fonts::{
    read::{
        collections::IntSet,
        tables::cmap::{
            Cmap, Cmap0, Cmap10, Cmap12, Cmap13, Cmap14, Cmap4, Cmap6, CmapSubtable, DefaultUvs, EncodingRecord,
            NonDefaultUvs, PlatformId, SequentialMapGroup, UvsMapping, VariationSelector,
        },
...
fn serialize_cmap14(
    ...
    for record in cmap14.var_selector().iter().rev() {
        if !plan.unicodes.contains(record.var_selector().to_u32()) {
            continue;
        }
    }
    Ok(())
...
fn copy_var_selector_record(
    ...
    if default_uvs_obj_idx.is_none() && non_default_uvs_obj_idx.is_none() {
        s.revert_snapshot(snap);
    }
...
fn copy_default_uvs(
    ...
    if org_num_range > plan.unicodes.len() as usize * num_bits {
        ...
        let last_code = INVALID_UNICODE_CHAR;
    }
