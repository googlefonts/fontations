...
use write_fonts::{
    read::{
        ...
        tables::{
            colr::{
                BaseGlyph, BaseGlyphList, BaseGlyphPaint, Clip, ClipBox, ClipBoxFormat1,
                ClipBoxFormat2, ClipList, ColorLine, ColorStop, Colr, Layer, LayerList, Paint,
...
impl Subset for Colr<'_> {
    fn subset(
        ...
        let Some(Ok(base_glyph_list)) = self.base_glyph_list() else {
            return Err(SubsetError::SubsetTableError(Colr::TAG));
        };
        Offset32::serialize_subset(&base_glyph_list, s, plan, (), 14)
            .map_err(|_| SubsetError::SubsetTableError(Colr::TAG))?;
...
fn serialize_v0(
    ...
    if num_records as usize > glyph_set.len() as usize * num_bit_storage {
        ...
        retained_record_idxes = base_glyph_records
            .iter()
            .enumerate()
...
impl<'a> SubsetTable<'_> for Paint<'a> {
    type ArgsForSubset = ();
    type Output = ();
