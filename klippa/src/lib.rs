...
const CODEPOINT_INVALID: u32 = 0xFFFFFFFF;
...
impl Plan {
    pub fn empty() -> Self {
        Self {
            unicodes: IntSet::empty(),
            glyphset_gsub: IntSet::empty(),
            glyphset: IntSet::empty(),
            num_h_metrics: 0,
            num_output_glyphs: 0,
            font_num_glyphs: 0,
            unicode_to_new_gid_list: Vec::new(),
            codepoint_to_glyph: HashMap::new(),
        }
    }

    pub fn new(input_gids: &IntSet<GlyphId>, input_unicodes: &IntSet<u32>, font: &FontRef) -> Self {
        let mut this = Plan::empty();
        // store font's num of glyphs in the plan
        this.font_num_glyphs = get_font_num_glyphs(font);
    ...
    pub fn populate_unicodes_to_retain(
        ...
        if input_gids.is_empty() && input_unicodes.len() < self.font_num_glyphs {
            ...
            let mut vec_cap = input_gids.len() + input_unicodes.len();
            vec_cap = vec_cap.min(cmap_unicodes.len());
    ...
    pub fn populate_gids_to_retain(&mut self, font: &FontRef) {
        //not-def
        self.glyphset_gsub.insert(GlyphId::from(0));
...
fn remove_invalid_gids(gids: &mut IntSet<GlyphId>, num_glyphs: usize) {
    gids.remove_range(GlyphId::new(num_glyphs as u16)..=GlyphId::new(CODEPOINT_INVALID as u16));
}

fn get_font_num_glyphs(font: &FontRef) -> usize {
    let mut ret = 0;
    let loca = font.loca(None).expect("Error reading loca table");
    ret = ret.max(loca.len());
