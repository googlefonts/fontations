...
use write_fonts::{
    read::{
        ...
        tables::layout::{
            ...
            CoverageTable, DeltaFormat, Device, DeviceOrVariationIndex, Feature, FeatureParams,
            SizeParams, StylisticSetParams, VariationIndex,
        },
...
impl<'a> SubsetTable<'a> for ClassDef<'a> {
    // remap_class: bool, keep_empty_table: bool, use_class_zero: bool, glyph_filter:Option<&Coverage>
    type ArgsForSubset = (bool, bool, bool, Option<&'a CoverageTable<'a>>);
...
impl<'a> SubsetTable<'a> for ClassDefFormat1<'a> {
    ...
    fn subset(
        ...
        let (remap_class, keep_empty_table, use_class_zero, glyph_filter) = args;
        ...
        for g in start..=end {
            ...
            if let Some(glyph_filter) = glyph_filter {
        ...
        let use_class_zero = if use_class_zero {
            let glyph_count = if let Some(glyph_filter) = glyph_filter {
                glyph_map
                    .keys()
        ...
        if !keep_empty_table && new_gid_classes.is_empty() {
...
impl<'a> SubsetTable<'a> for ClassDefFormat2<'a> {
    ...
    fn subset(
        ...
        if population as u64 > glyph_set.len() * num_bits / 2 {
...
mod test {
    ...
    fn test_subset_gpos_classdefs() {
        ...
        let ret = class_def1.subset(&plan, &mut s, (true, true, true, Some(&coverage)));
        assert!(ret.is_ok());
        assert!(!s.in_error());
        s.end_serialize();
        ...
        let mut s = Serializer::new(1024);
        assert_eq!(s.start_serialize(), Ok(()));
        let ret = class_def2.subset(&plan, &mut s, (true, true, false, None));
        assert!(ret.is_ok());
        assert!(!s.in_error());
        s.end_serialize();
    ...
    fn test_subset_gdef_glyph_classdef() {
        ...
        let ret = glyph_class_def.subset(&plan, &mut s, (false, false, true, None));
        assert!(ret.is_ok());
