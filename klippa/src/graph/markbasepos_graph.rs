...
pub(crate) fn split_markbase_pos(
    ...
    let Some(child_table_idxes) = get_table_child_idxes(graph, table_idx) else {
        return Ok(Vec::new());
    };

    let (class_mark_indices, mark_classes) =
        get_class_mark_indices_map(graph, table_idx, child_table_idxes.mark_array_idx)?;
    let split_points = compute_split_points(
        graph,
        table_idx,
        child_table_idxes.base_coverage_idx,
        child_table_idxes.mark_array_idx,
        &class_mark_indices,
    )?;
    ...
    let base_count =
        BaseArray::from_graph(graph, child_table_idxes.base_array_idx, mark_class_count)?
            .base_count() as usize;
    let mark_cov_glyphs = coverage_glyphs(graph, child_table_idxes.mark_coverage_idx)?;
    ...
    for i in 0..split_points.len() {
        ...
        let new_idx = clone_range(
            ...
            child_table_idxes.base_coverage_idx,
            child_table_idxes.mark_array_idx,
            child_table_idxes.base_array_idx,
            base_count,
            mark_class_count,
            start as u16,
            end as u16,
    ...
    shrink(
        graph,
        child_table_idxes.mark_coverage_idx,
        child_table_idxes.mark_array_idx,
        child_table_idxes.base_array_idx,
        base_count,
        &mark_classes,
...
fn get_table_child_idxes(graph: &Graph, table_idx: ObjIdx) -> Option<ChildTableIdxes> {
