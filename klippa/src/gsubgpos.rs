...
fn serialize_class_sequence(
    ...
    for c in sequence {
        if let Some(new_c) = class_map.get(&c.get()) {
            s.embed(*new_c)?;
        } else {
            return Err(SerializeErrorFlags::SERIALIZE_ERROR_EMPTY);
        }
...
impl<'a> SubsetTable<'a> for ChainedClassSequenceRule<'_> {
    ...
    fn subset(
        ...
        let (lookup_map, backtrack_class_map, input_class_map, lookahead_class_map) = args;
        // backtrack glyph count
        s.embed(self.backtrack_glyph_count())?;
        // backtrack sequence
        serialize_class_sequence(self.backtrack_sequence(), backtrack_class_map, s)?;

        // input glyph count
        s.embed(self.input_glyph_count())?;
        // input sequence
        serialize_class_sequence(self.input_sequence(), input_class_map, s)?;

        // lookahead glyph count
        s.embed(self.lookahead_glyph_count())?;
        // lookahead sequence
        serialize_class_sequence(self.lookahead_sequence(), lookahead_class_map, s)?;
