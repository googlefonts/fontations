...
fn split_lookup_subtables_if_needed(
    ...
    for i in 0..num_subtables as u32 {
        ...
        if let Some(new_subtables) = visited.get(&subtable_idx) {
        ...
        if new_subtables.is_empty() {
            visited.insert(subtable_idx, new_subtables);
            continue;
        }
...
pub(crate) mod test {
    ...
    fn populate_serializer_with_large_ligsubst(
        ...
        for l in 0..lig_subst_count {
            ...
            for i in 0..liga_set_count {
                add_offset(s, liga_set[i]);
            }

            liga_subst_idxes[l] = s.pop_pack(shared).unwrap();
        }

        for l in 0..lig_subst_count {
            liga_subst_idxes[l] = add_extension(s, liga_subst_idxes[l], 4);
        }
    }
    ...
    fn populate_serializer_with_shared_large_liga(
        ...
        for l in 0..lig_subst_count {
            add_offset(s, liga_subst_idxes[l]);
        }
