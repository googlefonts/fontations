...
fn promote_extensions_if_needed(
    ...
    for lookup_idx in lookups {
        let Some(lookup_v) = graph.vertex(*lookup_idx) else {
    ...
    let Some(lookup_list_v) = graph.vertex(lookup_list_idx) else {
        return Err(RepackErrorFlags::GRAPH_ERROR_INVALID_OBJ_INDEX);
    };
    ...
    for l in &lookup_sizes {
        ...
        if !layers_full {
            ...
            visited.clear();
            let subtables_size = graph.find_subgraph_size(l.obj_idx, &mut visited, 1) - lookup_size;
...
fn find_lookup_indices(graph: &Graph) -> Result<(ObjIdx, Vec<ObjIdx>), RepackErrorFlags> {
    ...
    let Some(lookup_list_idx) = graph.index_for_position(graph.root_idx(), 8) else {
        return Err(RepackErrorFlags::GRAPH_ERROR_INVALID_OBJ_INDEX);
    };
...
pub(crate) mod test {
    ...
    fn add_lookup_list(s: &mut Serializer, lookup_count: usize, lookups: &[ObjIdx]) -> ObjIdx {
        ...
        assert!(lookup_count <= lookups.len());
        for i in 0..lookup_count {
            add_offset(s, lookups[i]);
        }
    ...
    fn start_lookup(s: &mut Serializer, lookup_type: u8, num_subtables: u8) {
        let lookup_bytes = [0, 1, 0, 0, 0, num_subtables];
