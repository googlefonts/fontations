...
impl<T: Into<u32>> Bitset<T>  {
  ...
  fn contains(&self, val: T) -> bool {
    let val = val.into();
    self.page_for(val).map(|page| page.contains(val)).unwrap_or(false)
  }
  ...
  fn page_for(&self, value: u32) -> Option<&Page> {
    ...
    self.page_map.binary_search_by(|probe| probe.first_value.cmp(&first_value))
    .ok()
    .and_then(|info_idx| {
  ...
  fn page_for_mut(&mut self, value: u32) -> &mut Page {
    let first_value = self.get_first_value(value);
    match self.page_map.binary_search_by(|probe| probe.first_value.cmp(&first_value)) {
...
impl Page {
  ...
  fn iter(&self) -> impl Iterator<Item=u32> + '_ {
    self.storage.iter().enumerate()
      ...
      .flat_map(|(i, elem)| iter_bit_indices(*elem).map(move |idx| (i as u32 * ELEM_BITS) + idx))
  }
