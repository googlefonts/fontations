...
use read_fonts::test_data::gsub as test_data;
...
fn singlesubstformat1() {
    ...
    assert_hex_eq!(test_data::SINGLESUBSTFORMAT1_TABLE.as_ref(), &dumped);
}
...
fn singlesubstformat2() {
    ...
    let dumped = crate::write::dump_table(&table).unwrap();
    assert_hex_eq!(test_data::SINGLESUBSTFORMAT2_TABLE.as_ref(), &dumped);
}
...
fn multiplesubstformat1() {
    ...
    let dumped = crate::write::dump_table(&table).unwrap();
    assert_hex_eq!(test_data::MULTIPLESUBSTFORMAT1_TABLE.as_ref(), &dumped);
}
...
fn alternatesubstformat1() {
    ...
    let dumped = crate::write::dump_table(&table).unwrap();
    assert_hex_eq!(test_data::ALTERNATESUBSTFORMAT1_TABLE.as_ref(), &dumped);
}
...
fn ligaturesubstformat1() {
    ...
    let dumped = crate::write::dump_table(&table).unwrap();
    assert_hex_eq!(test_data::LIGATURESUBSTFORMAT1_TABLE.as_ref(), &dumped);
}
