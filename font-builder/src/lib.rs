...
use font_types::{Tag, CFF_SFNT_VERSION, TT_SFNT_VERSION};
...
impl<'a> FontBuilder<'a> {
    ...
    pub fn build(&mut self) -> Vec<u8> {
        ...
        for tag in table_order.iter() {
            ...
            position += padding;
            table_records.push(TableRecord { tag: *tag, checksum, offset, length });
        }
        table_records.sort_unstable_by_key(|record| record.tag);

        let is_cff = table_records.iter().any(|rec| [CFF, CFF2].contains(&rec.tag));
        let sfnt = if is_cff { CFF_SFNT_VERSION } else { TT_SFNT_VERSION };
        let num_tables = table_records.len();
        let entry_selector = (num_tables as f64).log2().floor() as usize;
        let search_range = (2.0_f64.powi(entry_selector as i32) * TABLE_RECORD_LEN as f64) as usize;
        let range_shift = (num_tables * TABLE_RECORD_LEN).saturating_sub(search_range);

        let mut data = Vec::with_capacity(position as usize);
        data.extend_from_slice(&sfnt.to_be_bytes());
        data.extend_from_slice(&(num_tables as u16).to_be_bytes());
        data.extend_from_slice(&(search_range as u16).to_be_bytes());
        data.extend_from_slice(&(entry_selector as u16).to_be_bytes());
        data.extend_from_slice(&(range_shift as u16).to_be_bytes());

        for record in &table_records {
            data.extend_from_slice(&record.tag.into_bytes());
            data.extend_from_slice(&record.checksum.to_be_bytes());
            data.extend_from_slice(&record.offset.to_be_bytes());
            data.extend_from_slice(&record.length.to_be_bytes());
        }
...
mod tests {
    ...
    fn validate_font_checksum() {
        ...
        let head = write_fonts::tables::head::Head::default();
        let head_data = write_fonts::dump_table(&head).unwrap();
        builder.add_raw(Tag::new(b"head"), head_data);

        for tag in [Tag::new(b"FOO "), Tag::new(b"BAR ")] {
    ...
    #[test]
    fn recommended_table_order_ttf() {
        check_recommended_table_order(&RECOMMENDED_TABLE_ORDER_TTF);
    }

    #[test]
    fn recommended_table_order_cff() {
        check_recommended_table_order(&RECOMMENDED_TABLE_ORDER_CFF);
    }

    fn check_recommended_table_order(recommended_order: &[Tag]) {
        let dsig = Tag::new(b"DSIG");
        let mut builder = FontBuilder::default();
