...
impl FreeTypeInstance {
    pub fn new(library: &Library, data: &SharedFontData, options: &InstanceOptions) -> Option<Self> {
        let mut face = library.new_memory_face2(data.clone(), options.index as isize).ok()?;
