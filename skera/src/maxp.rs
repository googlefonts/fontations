...
fn update_instancing_fields(
    ...
    s.copy_assign(
        maxp.shape().max_points_byte_range().start,
        head_maxp_info.max_points,
    );
    s.copy_assign(
        maxp.shape().max_contours_byte_range().start,
        head_maxp_info.max_contours,
    );
    s.copy_assign(
        maxp.shape().max_composite_points_byte_range().start,
        head_maxp_info.max_composite_points,
    );
    s.copy_assign(
        maxp.shape().max_composite_contours_byte_range().start,
        head_maxp_info.max_composite_contours,
    );
    s.copy_assign(
        maxp.shape().max_component_elements_byte_range().start,
        head_maxp_info.max_component_elements,
    );
    s.copy_assign(
        maxp.shape().max_component_depth_byte_range().start,
