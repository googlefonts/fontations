#![allow(dead_code)] // Will be used later as we build out variations support
...
impl Triple {
    pub(crate) fn new(minimum: f64, middle: f64, maximum: f64) -> Self {
        Self {
            minimum,
            middle,
            maximum,
...
type RebaseTentResultItem = (f64, Triple);
...
fn support_scalar(coord: f64, tent: Triple) -> f64 {
...
fn solve(tent: Triple, axis_limit: Triple, negative: bool) -> RebaseTentResult {
    ...
    if gain >= out_gain {
        ...
        if false && (new_upper <= axis_def + (axis_max - axis_def) * 2.0) {
            upper = new_upper;
            if !negative && axis_def + (axis_max - axis_def) * MAX_F2DOT14_F64 < upper {
                upper = axis_def + (axis_max - axis_def) * MAX_F2DOT14_F64;
...
pub(crate) fn rebase_tent(
    ...
    debug_assert!(tent.middle.abs() >= f64::EPSILON, "tent middle was zero",);
