...
#### <a id="font-read-args"></a> `FontRead`

After generating the type definitions, the next thing we generate is an
implementation of [`FontRead`][]. Via its [`ReadArgs`][] supertrait, this carries
an associated `Args` type, describing any external state required to read the type.
Most tables are
self-describing: that is, the data in the table can be fully interpreted without
any external information. These use `()` as their `Args`, and get the
argument-less `read` constructor for free. In some cases, however, this is not
possible, and a table requires a non-empty `Args`. A simple example is the
[`loca` table][loca-spec]:
...
default only work with types whose `Args` are `()`. For types that require
args, you can use the `#[read_offset_with($arg1, $arg1)]` attribute to indicate
that this offset needs to be resolved by passing the specified arguments to
`read_with_args`; these can be either the names of fields on the containing
table, or the name of arguments passed into this table through its *own* `Args`.
...
struct, as well as an implementation of `FontRead` with a non-empty `Args`
(discussed in the table section.) This looks like,
...
[`ReadArgs`]: https://docs.rs/read-fonts/latest/read_fonts/trait.ReadArgs.html
